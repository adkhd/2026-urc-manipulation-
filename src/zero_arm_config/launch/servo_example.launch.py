...
def generate_launch_description():
    moveit_config = (
        MoveItConfigsBuilder("zero_arm", package_name="zero_arm_config")
        .robot_description(file_path="config/zero_arm.urdf.xacro")
        .to_moveit_configs()
    )

    # Get parameters for the Servo node
    servo_yaml = load_yaml("zero_arm_config", "config/zero_servo.yaml")
    servo_params = {"moveit_servo": servo_yaml}

    # RViz
    rviz_config_file = (
        get_package_share_directory("zero_arm_config") + "/config/zero_rviz.rviz"
    )
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
    ...
    ros2_controllers_path = os.path.join(
        get_package_share_directory("zero_arm_config"),
        "config",
    ...
    container = ComposableNodeContainer(
        ...
        composable_node_descriptions=[
            ...
            ComposableNode(
                ...
                name="joy_node",
                parameters=[{
                "deadzone": 0.1,        # <- 여기서 데드존 줌 (축 최대값의 5%)
                "autorepeat_rate": 0.0,  # 필요 없으면 0
