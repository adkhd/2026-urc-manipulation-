...
# j4/j5 캐시 (한 콜백 사이클에서 q4,q5 둘 다 모았을 때만 명령)
    ...
    def _handle_velocity_axis(self, i, node, qdot_radps: float):
        ...
        # --- 단일 축 J1 ---
        if i == 0:
            rpm_raw = qdot_radps * RADPS_TO_RPM * 285 * 2
        ...
        if i == 4:
            ...
            for j in range(5):
                ...
                # 계수
                acc = max(abs(int(rpm_scaled // 1.5)), 200)
                dcc = max(abs(int(rpm_scaled // 1.3)), 300)
    ...
    def _handle_position_axis(self, i, node, q_rad: float):
        ...
        def calc_dynamic_params(axis_idx, target_cnt):
            ...
            vel = dist/pulse_quad/60./vel_t
            acc = vel/acc_t
            dcc = vel/dcc_t
            # 거리 5000 count당 Max속도 도달하도록 설정 (예시)
            
            # 너무 느리면 안되므로 최소 10RPM은 보장
            target_vel = max(10.0, vel[axis_idx])
            if axis_idx in(1,2) :
                target_vel = min(target_vel, 1500) # 리밋 클램핑
            else:
                target_vel = min(target_vel,3000)

            # 가속도는 속도에 비례하거나, 고정값을 사용
            # 여기서는 속도가 빠르면 가속도도 높게 설정
            target_acc = max(100.0,  acc[axis_idx])
            target_dcc = max(100.0,  dcc[axis_idx])
        ...
        if i == 4:
            ...
            if node_j4 is not None:
                v4, a4 = calc_dynamic_params(3, cnt3_target)
                drv.pos_control(node_j4, pos=cnt3_target, vel=v4, acc=a4, dcc=a4)

            # J5 모터 (Axis 4)
            if node_j5 is not None:
                v5, a5 = calc_dynamic_params(4, cnt4_target)
                drv.pos_control(node_j5, pos=cnt4_target, vel=v5, acc=a5, dcc=a5)
    ...
    def _publish_joint_state(self):
        ...
        for i in range(self.num_axes+1):
            if i == 0:
                pos[i] = pos_counts[i]/285./2./4./19.*2*math.pi
            elif i == 1:
